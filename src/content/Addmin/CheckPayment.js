  ...
  return (
    ...
    <div class=" flex justify-center items-center bg-white  font-prompt">
        <div class="overflow-x-auto  xl:w-[560px]  xl:h-[450px] md:w-[450px] sm:w-[400px]">
                ...
                {data.map((data)=>
                      ...
                      <td class="p-3 text-sm font-light whitespace-nowrap text-center">
                        <button                         
                          ...
                          onClick={()=>{changeContent(data);clickDetail();changeID(data.orderID,data.customerID);changeMoney(data.Money); }}
                        >                   
                          ดูรายละเอียด
                        </button>
                      </td>
                      
                      <td class={`p-3 text-sm font-light whitespace-nowrap text-center ${setState(data.orderID)[0]=== 1?"text-green-500":setState(data.orderID)[0]=== 0?"text-red-500":"text-gray-500"}`}>
                        {setState(data.orderID)[0]=== 1?"สำเร็จ":setState(data.orderID)[0]=== 0?"ไม่สำเร็จ":"ยังไม่ตรวจสอบ"}
                      </td>
                      
                      <td class="p-3 text-sm font-light whitespace-nowrap text-center ">
                        <button onClick={()=>handleDelete(data.orderID)}>X</button>
                      </td>
          ...
          {modalOn && <DetailPaymaent setModalOn={setModalOn} data={detail} id={id} customerID={idCustomer} money={money} setApprove={setApprove} approve={approve} setState={setState} />}
